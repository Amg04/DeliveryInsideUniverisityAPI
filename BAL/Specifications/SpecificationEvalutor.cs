...
{
    // helper
    public static class SpecificationEvalutor<TEntity> where TEntity : class, IModelBase
    {
        public static IQueryable<TEntity> GetQuery(IQueryable<TEntity> input , ISpecification<TEntity> spec)
        {
            ...
            // new 
            query = spec.IncludeQueries.Aggregate(query, (current, includeQuery) => includeQuery(current));
